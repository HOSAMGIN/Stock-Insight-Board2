...
// yahoo-finance2 v2 supports CJS require directly
let yahooFinance;
try {
  const yf = require("yahoo-finance2");
  yahooFinance = yf.default ?? yf;
  console.log("yahoo-finance2 loaded:", typeof yahooFinance);
} catch (e) {
...
const SYMBOL_NAMES = { SOXL:"Direxion Daily Semiconductors Bull 3X", TSLL:"Direxion Daily TSLA Bull 2X", NVDA:"NVIDIA Corporation", TSLA:"Tesla, Inc.", "000660.KS":"SK하이닉스", "005930.KS":"삼성전자", "005380.KS":"현대자동차", "^KS11":"KOSPI", "^GSPC":"S&P 500", "^NDX":"NASDAQ 100" };
const SYMBOL_DISPLAY = { SOXL:"SOXL", TSLL:"TSLL", NVDA:"NVDA", TSLA:"TSLA", "000660.KS":"하이닉스", "005930.KS":"삼성전자", "005380.KS":"현대차", "^KS11":"KOSPI", "^GSPC":"S&P500", "^NDX":"NDX100" };
const SYMBOL_CATEGORY = { SOXL:"us-stocks", TSLL:"us-stocks", NVDA:"us-stocks", TSLA:"us-stocks", "000660.KS":"kr-stocks", "005930.KS":"kr-stocks", "005380.KS":"kr-stocks", "^KS11":"indices", "^GSPC":"indices", "^NDX":"indices" };
...
function calcEMA(closes, period) {
  const k=2/(period+1); const emas=[closes[0]];
...
function calcMACD(closes) {
  ...
  const sv=calcEMA(mv,9); const l=mv.length-1;
  const ml=Math.round(mv[l]*10000)/10000, sl2=Math.round(sv[l]*10000)/10000;
  return {macdLine:ml, signalLine:sl2, histogram:Math.round((ml-sl2)*10000)/10000};
}
function detectCross(m20, m60) {
  const len=Math.min(m20.length,m60.length); if (len<2) return "none";
...
function buildHistory(sorted) {
  ...
  return sorted.slice(-30).map(d=>{
    ...
    const ml=Math.round(macdAll[gi]*10000)/10000, sl2=Math.round(sigAll[gi]*10000)/10000;
    const p20=gi>0?ma20S[gi-1]:null, p60=gi>0?ma60S[gi-1]:null, c20=ma20S[gi], c60=ma60S[gi];
    return {
      date:(d.date instanceof Date?d.date:new Date(d.date)).toISOString().split("T")[0],
      close:Math.round(d.close*100)/100, ma20:Math.round(s20*100)/100, ma60:Math.round(s60*100)/100,
      deviationPercent:Math.round((s20?((d.close-s20)/s20)*100:0)*100)/100,
      bbUpper:Math.round(bb.upper*100)/100, bbMiddle:Math.round(bb.middle*100)/100, bbLower:Math.round(bb.lower*100)/100,
      macdLine:ml, signalLine:sl2, macdHistogram:Math.round((ml-sl2)*10000)/10000,
...
async function fetchStockData(symbol) {
  if (!yahooFinance) throw new Error("yahoo-finance2 module not available");
  ...
  const sorted=historical.filter(d=>d.close!=null)
  ...
  const cur=quote.regularMarketPrice??closes[closes.length-1]??0;
  const prev=quote.regularMarketPreviousClose??closes[closes.length-2]??0;
  const bb=calcBB(closes,20)??{upper:ma20Val,middle:ma20Val,lower:ma20Val};
  const macd=calcMACD(closes);
  const m20S=closes.map((_,i)=>calcSMA(closes.slice(0,i+1),20));
  const m60S=closes.map((_,i)=>calcSMA(closes.slice(0,i+1),60));
  const cross=detectCross(m20S,m60S);
  return {
    symbol, displaySymbol:SYMBOL_DISPLAY[symbol]??symbol, name:SYMBOL_NAMES[symbol]??symbol,
    category:SYMBOL_CATEGORY[symbol]??"us-stocks",
    currentPrice:Math.round(cur*100)/100, previousClose:Math.round(prev*100)/100,
    changePercent:Math.round((prev?((cur-prev)/prev)*100:0)*100)/100,
    rsi14, rsiSignal:rsi14<30?"buy":rsi14>=70?"sell":"neutral",
    ma20:Math.round(ma20Val*100)/100, ma60:Math.round(ma60Val*100)/100,
    ma20DeviationPercent:Math.round((ma20Val?((cur-ma20Val)/ma20Val)*100:0)*100)/100,
    bbUpper:Math.round(bb.upper*100)/100, bbMiddle:Math.round(bb.middle*100)/100, bbLower:Math.round(bb.lower*100)/100,
    isTouchingLowerBand:cur<=bb.lower, isSuperBuySignal:cur<=bb.lower&&rsi14<30,
    macdLine:macd.macdLine, signalLine:macd.signalLine, macdHistogram:macd.histogram,
    crossSignal:cross, isBestTiming:rsi14<30&&cross==="golden",
    historicalPrices:buildHistory(sorted),
    lastUpdated:new Date().toISOString(),
    volume:quote.regularMarketVolume??0,
    currency:quote.currency??(symbol.endsWith(".KS")?"KRW":"USD"),
  };
}

let cache=null; const CACHE_TTL=60000;
const dynCache=new Map(); const DYN_TTL=120000;
...
async function getAllStocks() {
  ...
  if (data.length===0) throw new Error("All symbols failed. Sample error: "+errors[0]);
...
app.get("/api/healthz", (_req,res)=>res.json({status:"ok", yahooFinanceLoaded:!!yahooFinance, time:new Date().toISOString()}));
...
app.get("/", (_req,res)=>res.json({
  ...
  yahooFinanceLoaded:!!yahooFinance,
  endpoints:["/api/healthz","/api/stocks","/api/stocks/:symbol"],
}));
